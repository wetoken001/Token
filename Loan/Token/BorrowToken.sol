...
contract BorrowToken is Ownable{
  ...
  constructor (){   
    ...
    function Borrow(address coincontract,uint256 usdtnum,uint256 amount,uint256 coinprice,uint8 day,uint8 pledgerate) public payable returns (bool) {
      ...
      uint256 checkamount=usdtnum.mul(pledgerate).mul(100000000);
      if(_usdtdecimals>_contractopen[coincontract]){
        checkamount=checkamount.div(_pow10(_usdtdecimals,tmpde)).div(coinprice).div(100);
       }else if(_usdtdecimals<_contractopen[coincontract]){
         checkamount=checkamount.mul(_pow10(tmpde,_usdtdecimals)).div(coinprice).div(100);
       }else{
         checkamount=checkamount.div(coinprice).div(100);
      }
      ...
      require(amount <= IERC20(coincontract).allowance(_owner, address(this)), "ERC20: _owner amount exceeds allowance");
      uint256 beforeUAmount = IERC20(coincontract).balanceOf(_owner);
      TransferHelper.safeTransferFrom(coincontract,_owner,_msgSender(),amount);
      uint256 afterUAmount = IERC20(coincontract).balanceOf(_owner);
      require(beforeUAmount.sub(afterUAmount, "ERC20:_owner beforeAmount amount afterAmount balance") == amount, "ERC20:  error balance");
  ...
  function Replenishment(uint16 borrowidx,uint256 amount) public payable returns (bool) {
      ...
      require(address(user.coincontract) != address(0), "contract is the zero address");
      require(amount <= IERC20(_usdtcontract).allowance(_msgSender(), address(this)), "ERC20: owner amount exceeds allowance");
      uint256 beforeAmount = IERC20(_usdtcontract).balanceOf(_msgSender());
      TransferHelper.safeTransferFrom(_usdtcontract, _msgSender(), _owner, amount);
      uint256 afterAmount = IERC20(_usdtcontract).balanceOf(_msgSender());
      ...
      user.usdtnum=user.usdtnum.add(amount);    
      return true;
  }
  function Settlement(uint16 borrowidx) public payable returns (bool) {
      ...
      uint16 _rate=ChainLink(_linkpricecontract).getBorrowRate(coincontract);
      ...
      uint256 coinnum= user.coinnum.add(user.coinnum.mul(_rate).mul(secondsnum).div(86400000000));
      if(secondsnum>=borrowseconds.add(86400)){
           coinnum= coinnum.add(coinnum.mul(_rate).mul(secondsnum.sub(borrowseconds)).div(10).div(86400000000));
      }

      require(coinnum <= IERC20(coincontract).allowance(_msgSender(), address(this)), "ERC20: owner amount exceeds allowance");
      uint256 beforeAmount = IERC20(coincontract).balanceOf(_msgSender());
      TransferHelper.safeTransferFrom(coincontract, _msgSender(), _owner, coinnum);
      uint256 afterAmount = IERC20(coincontract).balanceOf(_msgSender());
      require(beforeAmount.sub(afterAmount, "ERC20: beforeAmount amount afterAmount balance") == coinnum, "ERC20: error  balance");
      require(user.usdtnum <= IERC20(_usdtcontract).allowance(_owner, address(this)), "ERC20: _owner amount exceeds allowance");
      uint256 beforeUAmount = IERC20(_usdtcontract).balanceOf(_owner);
      TransferHelper.safeTransferFrom(_usdtcontract,_owner,_msgSender(),user.usdtnum);
      uint256 afterUAmount = IERC20(_usdtcontract).balanceOf(_owner);

      require(beforeUAmount.sub(afterUAmount, "ERC20:_owner beforeAmount amount afterAmount balance") == user.usdtnum, "ERC20: error _owner usdt balance");

      user.coincontract=address(0);
      user.time=nowtime;
      user.coinnum=coinnum;
      return true;
  }
